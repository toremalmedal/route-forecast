...
if [ ! -d "./location-forecast-client/" ]; then
  ...
  docker run --rm -v "${PWD}:/local" openapitools/openapi-generator-cli generate -i /local/location-forecast-swagger-edit.json -g rust -o /local/location-forecast-client --package-name=location-forecast-client --additional-properties=supportMiddleware=true,library=reqwest-trait
...
if [ ! -d "./ors-client/" ]; then
  ...
  docker run --rm -v "${PWD}:/local" openapitools/openapi-generator-cli generate -i /local/ors-swagger-edit.json -g rust -o /local/ors-client --package-name=ors-client --additional-properties=supportMiddleware=true,library=reqwest-trait
...
if [ ! -d "./stedsnavn-client/" ]; then
  ...
  docker run --rm -v "${PWD}:/local" openapitools/openapi-generator-cli generate -i /local/stedsnavn.json -g rust -o /local/stedsnavn-client --package-name=stedsnavn-client --additional-properties=supportMiddleware=true,library=reqwest-trait
